...
public:
    ...
    virtual void DumpFile(const void* inputData, size_t inputDataSize, std::ostream& output) const = 0;
};

class AbstractStreamingFileTypeDumper : public IFileTypeDumper
{
protected:
    virtual void DumpFile(std::istream& input, std::ostream& output) const = 0;
