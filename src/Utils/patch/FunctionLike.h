...
public:
    ...
    template <class... _Var_Types>
    _Ret operator()(_Types ... args, _Var_Types ... varArgs) const
    {
        if (m_func == nullptr)
            throw std::exception("Tried accessing uninitialized function offset.");
...
template <typename T>
struct _Return_param_size
{
    static constexpr size_t SIZE = sizeof T;
};

template <>
struct _Return_param_size<void>
{
...
// CLASS TEMPLATE _Func_class
template <class _Ret, class... _Types>
class _Func_class_param_capture : public ParamCaptureFunc
{
...
template <class T>
struct _Get_function_impl_param_capture;
...
template <class T>
struct _Get_function_impl_varargs;

#define _GET_FUNCTION_IMPL(CALL_OPT, X1, X2, X3)                                                            \
    template <class _Ret, class... _Types>                                                                  \
    struct _Get_function_impl_varargs<_Ret CALL_OPT(_Types...)> /* determine type from argument list */     \
    {                                                                                                       \
