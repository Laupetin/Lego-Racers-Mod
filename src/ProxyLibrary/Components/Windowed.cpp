...
namespace windowed
{
    ...
    HWND __stdcall CreateWindowExHk(const DWORD dwExStyle,
                                    const LPCSTR lpClassName,
                                    const LPCSTR lpWindowName,
                                    [[maybe_unused]] DWORD dwStyle,
                                    [[maybe_unused]] int x,
                                    [[maybe_unused]] int y,
                                    [[maybe_unused]] int nWidth,
                                    [[maybe_unused]] int nHeight,
                                    HWND hWndParent,
                                    HMENU hMenu,
                                    HINSTANCE hInstance,
                                    LPVOID lpParam)
    {
        return CreateWindowExA(
            dwExStyle, lpClassName, lpWindowName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, hWndParent, hMenu, hInstance, lpParam);
    }
    ...
    bool ShouldDrawCursor(HWND hwnd)
    {
        ...
        return cursorPos.x >= rectTopLeft.x && cursorPos.x < rectBottomRight.x && cursorPos.y >= rectTopLeft.y && cursorPos.y < rectBottomRight.y;
    }
